...
void get_muscle_direction(Eigen::MatrixXd& v, int nx, int ny, int nn)
{
	...
	// int k = 16;
	// for (int yidx = 0 ; yidx < ny/2 ; yidx++)
	// {	
	// 	for (int xidx = 0 ; xidx < nx ; xidx++)
	// 	{
	// 		if(xidx > k && xidx < (nx - k))
	// 		{
	// 			isBoundary(yidx,xidx) = 1;
	// 			v(yidx,xidx) = 10;
	// 		}
	// 	}
	// 	k = k - 2;
...
// zero horizontal line
	for (int xidx = 0 ; xidx < 21 ; xidx++)
	{
		isBoundary(15,xidx) = 1;
		v(15,xidx) = -10;
	}

// top & down horizontal line
	for (int xidx = 15 ; xidx < nx-15 ; xidx++)
	{
		v(0,xidx) = 10;
		isBoundary(0,xidx) = 1;
	}
	...
	{
		v(ny-nn+i,i+20) = -10.;
		isBoundary(ny-nn+i,i+20) = 1;
	} 

	for (int i= 0; i < 6 ; i++)
	{
		v(ny-nn+i+9,nx-i-1) = -10.;
		isBoundary(ny-nn+i+9,nx-i-1) = 1;
	} 

	for(int yidx = 15; yidx < ny-6 ; yidx++)
	{
		v(yidx,nx-1) = -10.;
	...
	// //boundary exception
	// int x_flag = 0;
	// for(int yidx = 1; yidx < nn-1 ; yidx++)
	// 	{
	// 		for (int xidx = 1 ; xidx < nx-1 ; xidx++)
	// 	{
	// 		if (x_flag == 0 && isBoundary(yidx,xidx) != 1)
	// 		{
	// 			v(yidx, xidx) = 100;
	// 		}
	// 		else if(x_flag == 1 && isBoundary(yidx,xidx) == 1)
	// 			x_flag = 0;
	// 		else 
	// 			x_flag = 1;
	// 	}
	...
	// for(int yidx = nn+1; yidx < ny-1 ; yidx++)
	// 	{
	// 		for (int xidx = 1 ; xidx < nx-1 ; xidx++)
	// 	{
	// 		if (x_flag == 0 && isBoundary(yidx,xidx) != 1)
